...
---
title: "acic_final_run_practice_public"
output:
  html_document:
    toc: true
---
...
```{r}
# acic_2022 helper functions - practice level data input
...
acic_task_practice_launcher <- function(task_list) {
  env <- get_env()
  simple_caller <- function(dataset) {
    env <- get_env()
    if (env == "local") root <- "~/Projects/Data_Challenge_2022"
    if (env == "db") root <- "/dbfs/FileStore/acic_2022"
    acic_task_practice_caller(dataset, overwrite = T, n_boot = 500, root = root, group_ps = F)
  }
...
# wrapper to launch task on Databricks
acic_task_practice_caller <- function(dataset, root = "~/Projects/Data_Challenge_2022", version = "ipwdid_v2_practice",
                             overwrite = FALSE, n_boot = 20, refit = F, group_ps = F) {
  library(tidyverse)
  library(speedglm)
  library(boot)
  ...
  if (overwrite || !file.exists(output_file)){
    dfw <- load_practice_data(dataset, root = root)
    res <- fit_model_ipwdid_practice(dfw, max_weight = 5, group_ps = group_ps, method = "boot", boot_method = "basic", n_boot = n_boot, refit = refit)
    res <- res %>% 
...
load_practice_data <- function(
  ...
  env <- get_env()
  dataset_str <- stringr::str_pad(dataset, 4, pad = "0")
  file_list <- c("practice", "practice_year")
  type_list <- c("cififidddd", "cidiiiddddddd")
  ...
  tbl_list <- lapply(1:2, function(i) {
    file <- file_list[i]
    type <- type_list[i]
    # mount_file <- file.path(mount_root, file, paste0("acic_", file, "_", dataset_str, ".csv"))
    local_file <- file.path(root, file, paste0("acic_", file, "_", dataset_str, ".csv"))
  ...
  practice_year_df <- 
    ...
    left_join(tbl_list[[2]] %>% dplyr::select(id.practice, year, Y, post), by = "id.practice") %>%
    left_join(tbl_list[[1]], by = "id.practice")
    
  colnames(practice_year_df) <- gsub('\\.', '_', colnames(practice_year_df))  
  
  if (fix_neg) practice_year_df$Y <- pmax(practice_year_df$Y, 0.01)
  if (format == "wide") {
    # change to wide format 
    practice_year_df <- long_to_wide_practice(practice_year_df)
  }
  return(practice_year_df)
}

### add propensity score weights - practice level
...
```{r}
acic_task_practice_launcher(1)
```


```{r}
# launch task
acic_task_practice_launcher(1:3400)
```
...
combine_results <- function(
  root = "/FileStore/acic_2022",
  model = "final_norefit_cl"
) {
  require(tidyverse)
  require(SparkR)
  dataset <- 1:3400
  dataset_str <- stringr::str_pad(dataset, 4, pad = "0")
  output_dir <- paste0("/dbfs", file.path(root, "models", model))
  # check file completeness
  files <- list.files(output_dir)
  filenum_str <- stringr::str_extract(files, "[0-9]{4}")
  non_exist <- setdiff(dataset_str, filenum_str)
  if (length(non_exist) > 0) {
    print(non_exist)
  }
  read_res <- function(f) {
    readr::read_csv(file.path(output_dir, f), show_col_types = FALSE)
  }
  res_list <- spark.lapply(files, read_res)
...
```{r}
comb_df <- combine_results(model = 'ipwdid_v2_practice')
```
